  ...
  return (
    <div className={`flex flex-col items-center justify-center ${containerClasses[size]}`}>
      <div className={`relative ${orbitSize[size]}`}>
        ...
        <div className="absolute inset-0 rounded-full border-2 border-transparent border-t-purple-400 animate-spin" style={{ animationDuration: '1.5s' }} />
        <div className="absolute inset-0 animate-spin" style={{ animationDuration: '1.5s' }}>
          <div className="absolute -top-1 left-1/2 -translate-x-1/2">
            <Plane className={`${sizeClasses[size]} text-purple-400 rotate-90`} />
          </div>
        </div>
      </div>
