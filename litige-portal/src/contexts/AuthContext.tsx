...
// Clés localStorage spécifiques au litige portal
const TOKEN_KEY = 'bfs_litige_token';
const USER_KEY = 'bfs_litige_user';
  ...
  const checkAuth = async () => {
    try {
      ...
      if (resData.success) {
        ...
        // Vérifier que c'est bien un utilisateur baggage_dispute
        if (userData.role !== 'baggage_dispute') {
  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      if (resData.success) {
        ...
        // Vérifier que c'est bien un utilisateur baggage_dispute
        if (userData.role !== 'baggage_dispute') {
          throw new Error('Ce portail est réservé aux utilisateurs Litige Bagage. Veuillez utiliser le portail approprié à votre rôle.');
        }
