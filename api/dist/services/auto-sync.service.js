...
exports.clearSyncCache = clearSyncCache;
exports.autoSyncIfNeeded = autoSyncIfNeeded;
...
function clearSyncCache(airportCode) {
    if (airportCode) {
 ...
 */
async function autoSyncIfNeeded(airportCode) {
    try {
        ...
        const { count: unprocessedCount } = await database_1.supabase
            .from('raw_scans')
 ...
 */
async function performSyncInBackground(airportCode) {
    try {
                ...
                if (scan.scan_type === 'baggage_tag' && scan.baggage_rfid_tag && scan.status_baggage) {
                    ...
                    if (!existingBag) {
                        ...
                        if (passengerByBase) {
                            ...
                            const { data: allPassengers } = await database_1.supabase
                                .from('passengers')
