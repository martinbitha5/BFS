...
exports.pnrExtractorService = void 0;
...
class PnrExtractorService {
     ...
     */
    getAirportCodes() {
        return Array.from(airport_codes_1.KNOWN_AIRPORT_CODES);
    }
    /**
     ...
     */
    extractWithSpacePattern(rawData, airportCodes, candidates) {
     ...
     */
    extractEthiopianDirect(rawData, airportCodes, candidates) {
     ...
     */
    extractAirCongoDirect(rawData, airportCodes, candidates) {
     ...
     */
    extractKenyaAirwaysDirect(rawData, airportCodes, candidates) {
     ...
     * Format typique: M1[NOM] [PNR] [DEP][ARR][CODE] [VOL]...
     */
    extractOtherAirlinesDirect(rawData, airportCodes, candidates) {
        ...
        for (const match of matches) {
            ...
            if (airportCodes.includes(pnr) || airlineCode === depCode || airlineCode === arrCode)
                continue;
        ...
        for (const match of codeMatches) {
            ...
            if (airport_codes_1.KNOWN_AIRPORT_CODES.some(apt => apt === code))
                continue;
            candidates.push({
                pnr,
                confidence: 65,
     ...
     */
    extractGenericPatterns(rawData, airportCodes, candidates) {
        ...
        for (const match of genericMatches) {
            ...
            const context = rawData.substring(Math.max(0, position - 10), Math.min(rawData.length, position + 16));
            if (context.match(/^[A-Z]{20,}/))
                continue;
            candidates.push({
                pnr,
                confidence: 30,
...
// Instance singleton
exports.pnrExtractorService = new PnrExtractorService();
