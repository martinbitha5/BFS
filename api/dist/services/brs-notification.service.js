...
exports.brsNotificationService = void 0;
...
class BRSNotificationService {
     ...
     */
    async sendNotification(notificationId, data) {
     ...
     */
    async notifyException(exception, airportCode) {
        const severityMap = {
            low: 'info',
            medium: 'warning',
            high: 'error',
     ...
     */
    async notifyWorkflowStep(reportId, step, status, airportCode) {
        if (status === 'completed') {
            await this.createNotification({
                type: 'workflow',
                severity: 'info',
     ...
     */
    async notifyTransfer(transfer, airportCode) {
        await this.createNotification({
            type: 'transfer',
            severity: 'info',
     ...
     */
    async notifySLAViolation(reportId, delayHours, airportCode) {
        await this.createNotification({
            type: 'sla',
...
exports.brsNotificationService = new BRSNotificationService();
