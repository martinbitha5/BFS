...
const birs_parser_service_1 = require("../services/birs-parser.service");
...
router.get('/international-baggages', airport_restriction_middleware_1.requireAirportCode, async (req, res, next) => {
    try {
...
router.get('/reports', airport_restriction_middleware_1.requireAirportCode, async (req, res, next) => {
    try {
        const airportCode = req.userAirportCode; // Peut être undefined si accès total
        const airlineCode = req.query.airline_code; // Filtre optionnel par compagnie
        let query = database_1.supabase
            .from('birs_reports')
            .select('*');
        // Filtrer par aéroport uniquement si l'utilisateur n'a pas accès total
...
router.get('/reports/:id', airport_restriction_middleware_1.requireAirportCode, async (req, res, next) => {
    try {
        const { id } = req.params;
        const airportCode = req.userAirportCode; // Peut être undefined si accès total
        const airlineCode = req.query.airline_code; // Filtre optionnel par compagnie
...
router.get('/statistics/:airport', airport_restriction_middleware_1.requireAirportCode, async (req, res, next) => {
    try {
        ...
        const { data: baggages, error: baggagesError } = await database_1.supabase
            .from('international_baggages')
            .select('status')
        ...
        let reportsQuery = database_1.supabase
            .from('birs_reports')
            .select('*')
            .eq('airport_code', airport);
        // Filtrer par compagnie si spécifié
...
router.post('/upload', async (req, res, next) => {
    try {
        const { fileName, fileContent, reportType, flightNumber, flightDate, origin, destination, airline, airlineCode, airportCode } = req.body;
...
// RESTRICTION: Vérifie que le rapport appartient à l'aéroport de l'utilisateur et optionnellement par compagnie
router.post('/reconcile/:reportId', airport_restriction_middleware_1.requireAirportCode, async (req, res, next) => {
    try {
        const { reportId } = req.params;
        const { airportCode, airline_code } = req.body;
        const userAirport = req.query.airport || airportCode;
        if (!userAirport) {
            return res.status(400).json({
                success: false,
        ...
        let reportQuery = database_1.supabase
            .from('birs_reports')
        ...
        // Récupérer les items du rapport
        const { data: reportItems, error: itemsError } = await database_1.supabase
            .from('birs_report_items')
            .select('*')
        ...
        const { data: scannedBags, error: bagsError } = await database_1.supabase
            .from('international_baggages')
            .select('*')
        ...
        for (const item of reportItems || []) {
            ...
            const matchedBag = scannedBags?.find(bag => {
                ...
                if (bag.tag_number === item.bag_id)
                    return true;
                // Match sur PNR si disponible
                if (bag.pnr && item.pnr && bag.pnr === item.pnr)
                    return true;
...
router.post('/manual-reconcile', airport_restriction_middleware_1.requireAirportCode, async (req, res, next) => {
    try {
        const { itemId, baggageId, userId } = req.body;
        const airport = req.query.airport || req.body.airport_code;
        if (!airport) {
            return res.status(400).json({
                success: false,
                error: 'Code aéroport requis'
        ...
        const { data: item, error: itemError } = await database_1.supabase
            .from('birs_report_items')
            .select('*')
            .eq('id', itemId)
            .single();
        ...
        // Mettre à jour le bagage
        const { error: updateBaggageError } = await database_1.supabase
            .from('international_baggages')
            .update({
            status: 'reconciled',
            ...
            .eq('id', baggageId);
        if (updateBaggageError)
            throw updateBaggageError;
        // Mettre à jour l'item du rapport
        const { error: updateItemError } = await database_1.supabase
            .from('birs_report_items')
            .update({
        ...
        const { data: reportItems } = await database_1.supabase
            .from('birs_report_items')
...
router.get('/reports/:id/export', airport_restriction_middleware_1.requireAirportCode, async (req, res, next) => {
    try {
        const { id } = req.params;
        ...
        const { data: reportCheck, error: checkError } = await database_1.supabase
            .from('birs_reports')
            .select('airport_code')
            .eq('id', id)
            .single();
