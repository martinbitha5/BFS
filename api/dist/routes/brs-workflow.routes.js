...
const brs_notification_service_1 = require("../services/brs-notification.service");
...
router.get('/workflow/:reportId', airport_restriction_middleware_1.requireAirportCode, async (req, res, next) => {
    try {
        ...
        const { data: dbWorkflowSteps, error: workflowError } = await database_1.supabase
...
router.post('/workflow/:reportId/step/:stepId', async (req, res, next) => {
    try {
        ...
        const { data: workflowStep, error: stepError } = await database_1.supabase
            .from('brs_workflow_steps')
            .insert({
            report_id: reportId,
            step: stepId,
        ...
        if (stepError && stepError.code !== '42P01') {
            ...
            const { data: report } = await database_1.supabase
                .from('birs_reports')
                .select('airport_code')
                .eq('id', reportId)
                .single();
            if (report) {
                await brs_notification_service_1.brsNotificationService.notifyWorkflowStep(reportId, stepId, 'completed', report.airport_code);
            }
        }
...
router.get('/exceptions', airport_restriction_middleware_1.requireAirportCode, async (req, res, next) => {
    try {
        ...
        const { data: unmatchedBaggages, error: bagsError } = await database_1.supabase
        ...
        if (bagsError)
            throw bagsError;
        const { data: reports, error: reportsError } = await database_1.supabase
            .from('birs_reports')
            .select('*')
            .eq('airport_code', airport);
        if (reportsError)
        ...
        const { data: dbExceptions, error: exceptionsError } = await database_1.supabase
            .from('brs_exceptions')
            .select('*')
            .eq('airport_code', airport); // FORCER le filtre par aéroport
        if (exceptionsError) {
            ...
            const exceptions = unmatchedBaggages?.map(bag => ({
                id: `exc_${bag.id}`,
        ...
        const exceptions = dbExceptions?.map(exc => ({
            id: exc.id,
            type: exc.type,
...
router.post('/exceptions', airport_restriction_middleware_1.requireAirportCode, async (req, res, next) => {
    try {
        const { type, severity, baggage_id, report_item_id, flight_number, description, created_by, airport_code } = req.body;
        const userAirport = req.query.airport || airport_code;
        if (!userAirport) {
            return res.status(400).json({
                success: false,
                error: 'Code aéroport requis'
        ...
        const { data: exception, error: createError } = await database_1.supabase
            .from('brs_exceptions')
            .insert({
            type,
...
router.post('/exceptions/:exceptionId/resolve', async (req, res, next) => {
    try {
...
router.post('/rush', async (req, res, next) => {
    try {
        ...
        const { data: exception, error: exceptionError } = await database_1.supabase
            .from('brs_exceptions')
            .insert({
            type: 'rush',
            severity: 'high',
        ...
        if (exceptionError && exceptionError.code !== '42P01') {
            ...
            await brs_notification_service_1.brsNotificationService.notifyException(exception, airport_code || baggage.airport_code);
        }
        res.json({
            success: true,
...
// RESTRICTION: Filtre automatiquement par aéroport de l'utilisateur
router.get('/alerts', airport_restriction_middleware_1.requireAirportCode, async (req, res, next) => {
    try {
        ...
        const { data: oldReports } = await database_1.supabase
            .from('birs_reports')
            .select('*')
            .eq('airport_code', airport)
        ...
        const { data: lowReconciliationReports } = await database_1.supabase
            .from('birs_reports')
            .select('*')
            .eq('airport_code', airport)
        ...
        const { data: criticalUnmatched } = await database_1.supabase
            .from('international_baggages')
            .select('*')
            .eq('airport_code', airport)
            .eq('status', 'unmatched')
...
router.get('/traceability/:identifier', airport_restriction_middleware_1.requireAirportCode, async (req, res, next) => {
    try {
        ...
        const { data: baggageByTag } = await database_1.supabase
            .from('international_baggages')
            .select('*')
        ...
        if (baggageByTag) {
            ...
            const { data: baggageById, error } = await database_1.supabase
                .from('international_baggages')
                .select('*')
                .eq('id', identifier)
                .eq('airport_code', airport) // FORCER le filtre par aéroport
        ...
        if (baggage.birs_report_id) {
            const { data: reportData } = await database_1.supabase
                .from('birs_reports')
                .select('*')
...
router.get('/dashboard/:airport', airport_restriction_middleware_1.requireAirportCode, async (req, res, next) => {
    try {
        const { airport } = req.params;
        const userAirport = req.query.airport;
        ...
        const { data: allBaggages } = await database_1.supabase
            .from('international_baggages')
            .select('status, scanned_at')
            .eq('airport_code', airport);
        const { data: allReports } = await database_1.supabase
            .from('birs_reports')
            .select('*')
            .eq('airport_code', airport);
...
router.post('/transfers', async (req, res, next) => {
    try {
        const { baggage_id, from_flight_number, from_airport_code, to_flight_number, to_airport_code, transfer_type, connection_time_minutes, transfer_airport_code, requested_by, notes, reason } = req.body;
        // Créer le transfert
        const { data: transfer, error: transferError } = await database_1.supabase
            .from('brs_transfers')
            .insert({
            baggage_id,
            from_flight_number,
        ...
        const { data: baggage } = await database_1.supabase
            .from('international_baggages')
            .select('flight_number, airport_code')
            .eq('id', baggage_id)
            .single();
        if (baggage && transfer) {
            const { data: exception } = await database_1.supabase
                .from('brs_exceptions')
                .insert({
                type: 'transfer',
                severity: 'medium',
            ...
            // Envoyer une notification
            if (transfer) {
                await brs_notification_service_1.brsNotificationService.notifyTransfer(transfer, baggage.airport_code);
            }
        }
        res.status(201).json({
            success: true,
...
router.post('/transfers/:transferId/confirm', async (req, res, next) => {
    try {
