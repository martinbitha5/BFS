...
router.get('/stats', async (req, res) => {
    try {
        ...
        if (!isAllAccess)
            totalQuery = totalQuery.eq('airport_code', airportCode);
        ...
        if (!isAllAccess)
            todayQuery = todayQuery.eq('airport_code', airportCode);
        const { count: todayCount } = await todayQuery;
        // Logs par action (top 10)
        let actionQuery = supabase.from('audit_logs').select('action');
        if (!isAllAccess)
            actionQuery = actionQuery.eq('airport_code', airportCode);
        ...
        if (!isAllAccess)
            entityQuery = entityQuery.eq('airport_code', airportCode);
