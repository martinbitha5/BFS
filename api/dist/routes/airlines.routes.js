...
router.get('/', async (req, res, next) => {
    try {
        const { data: airlines, error } = await database_1.supabase
            .from('airlines')
...
router.post('/signup', async (req, res, next) => {
    try {
        ...
        const { data: existingAirline } = await database_1.supabase
            .from('airlines')
            .select('id')
...
router.post('/login', async (req, res, next) => {
    try {
        ...
        const { data: airline, error } = await database_1.supabase
            .from('airlines')
            .select('*')
            .eq('email', email)
            .single();
        if (error || !airline) {
            ...
            const { data: pendingRequest } = await database_1.supabase
                .from('airline_registration_requests')
                .select('id, status')
                .eq('email', email)
                .eq('status', 'pending')
        ...
        const validPassword = await bcrypt_1.default.compare(password, airline.password);
        if (!validPassword) {
            return res.status(401).json({
                success: false,
                error: 'Les identifiants saisis sont incorrects. Veuillez vérifier votre email et votre mot de passe.',
        ...
        const token = jsonwebtoken_1.default.sign({ id: airline.id, code: airline.code, email: airline.email }, JWT_SECRET, { expiresIn: '7d' });
