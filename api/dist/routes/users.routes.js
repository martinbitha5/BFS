...
router.get('/', async (req, res, next) => {
    try {
        const airportCode = req.query.airport || req.headers['x-airport-code'];
        if (!airportCode) {
            return res.status(400).json({ error: 'Code aéroport requis' });
        }
        let query = database_1.supabase.from('users').select('*');
...
router.put('/:id', async (req, res, next) => {
    try {
        ...
        if (password && password.length >= 6) {
            const { error: pwdError } = await database_1.supabase.auth.admin.updateUserById(id, {
                password
            });
            if (pwdError) {
...
router.delete('/:id', async (req, res, next) => {
    try {
        ...
        const { error: authDeleteError } = await database_1.supabase.auth.admin.deleteUser(id);
        if (authDeleteError) {
 ...
 */
router.post('/create-by-support', async (req, res, next) => {
    try {
        ...
        const { data: { user: authUser }, error: authError } = await database_1.supabase.auth.getUser(token);
        ...
        const { data: currentUser, error: userError } = await database_1.supabase
            .from('users')
        ...
        const { data: authData, error: createAuthError } = await database_1.supabase.auth.admin.createUser({
            email,
            password,
            email_confirm: true
        });
        ...
        const { data: userData, error: createUserError } = await database_1.supabase
            .from('users')
            .insert({
            id: authData.user.id,
            email,
