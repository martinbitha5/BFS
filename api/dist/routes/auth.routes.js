...
router.post('/register', async (req, res, next) => {
    try {
        ...
        if (authError) {
            return res.status(400).json({
                success: false,
                error: (0, auth_errors_util_1.normalizeAuthError)(authError.message, 'register')
            });
        }
        ...
        const { data: registrationRequest, error: requestError } = await database_1.supabase
            .from('user_registration_requests')
            .insert({
            email,
            full_name: name,
...
router.post('/login', async (req, res, next) => {
    try {
        ...
        const { data: authData, error: authError } = await database_1.supabase.auth.signInWithPassword({
            email,
            password
        });
        if (authError) {
            return res.status(401).json({
                success: false,
                error: (0, auth_errors_util_1.normalizeAuthError)(authError.message, 'login')
            });
        }
