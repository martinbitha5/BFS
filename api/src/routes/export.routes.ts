...
interface ParsedRawScan {
    ...
    // Données parsées (peut être null si parsing échoue)
    parsed: {
        pnr?: string;
        fullName?: string;
        flightNumber?: string;
    ...
    // Erreur de parsing (si applicable)
    parsingError?: string;
}
 ...
 */
router.get('/raw-scans', async (req: Request, res: Response) => {
    try {
        ...
        // Parser chaque scan
        const parsedScans: ParsedRawScan[] = (rawScans || []).map((scan) => {
            let parsed = null;
            let parsingError = undefined;

            try {
                // Tenter de parser les données brutes
                const parsedData = parserService.parse(scan.raw_data);
                parsed = {
                    pnr: parsedData.pnr,
