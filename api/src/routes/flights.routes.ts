...
router.put('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (updates.stops !== undefined) dbUpdates.stops = updates.stops; // Escales intermédiaires
...
router.post('/validate-boarding', async (req, res, next) => {
  try {
    ...
    console.log(`[ValidateBoarding] 📊 ${data.length} vol(s) trouvé(s):`, 
      data.map(f => `${f.flight_number} (${f.departure}->${f.arrival})`).join(', ')
    );
