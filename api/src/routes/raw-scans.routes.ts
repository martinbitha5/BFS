...
/**
 * GET /api/v1/raw-scans/parsed?airport=XXX&start_date=YYYY-MM-DD&end_date=YYYY-MM-DD
 * Récupérer les scans bruts PARSÉS (données structurées)
 * Utilise le parser.service pour extraire PNR, nom, vol, route, etc.
 */
router.get('/parsed', async (req: Request, res: Response) => {
    try {
        const { airport, start_date, end_date, scan_type } = req.query;

        if (!airport) {
            return res.status(400).json({ error: 'Le code aéroport est requis' });
        }

        // Récupérer les raw scans
        let query = supabase
            .from('raw_scans')
            .select('*')
            .eq('airport_code', airport)
            .eq('scan_type', 'boarding_pass') // Seulement les boarding pass
            .order('last_scanned_at', { ascending: false });
        ...
        // Parser chaque raw scan
        const parsedData = (rawScans || []).map((scan: any) => {
            try {
                ...
                return {
                    ...
                    status_checkin: scan.status_checkin || false,
                    status_baggage: scan.status_baggage || false,
                    status_boarding: scan.status_boarding || false,
                    status_arrival: scan.status_arrival || false,
                    // Données brutes (pour debug)
                    raw_data: scan.raw_data,
                    format: parsed.format,
                };
            } catch (parseError) {
                console.error('Error parsing raw scan:', parseError);
                // En cas d'erreur de parsing, renvoyer des données minimales
                return {
                    pnr: 'PARSE_ERROR',
                    full_name: 'PARSE_ERROR',
        ...
        console.error('Error in GET /raw-scans/parsed:', err);
        res.status(500).json({ error: err.message || 'Erreur serveur' });
    }
});
