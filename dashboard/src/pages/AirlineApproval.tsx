import { CheckCircle, Clock, XCircle, AlertCircle, UserCheck, UserX, ShieldAlert, Plane } from 'lucide-react';
import { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import api from '../config/api';
import { useAuth } from '../contexts/AuthContext';

interface AirlineRegistrationRequest {
  id: string;
  name: string;
  code: string;
  email: string;
...
export default function AirlineApproval() {
  ...
  const [requests, setRequests] = useState<AirlineRegistrationRequest[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [filter, setFilter] = useState<'all' | 'pending' | 'approved' | 'rejected'>('pending');
  const [selectedRequest, setSelectedRequest] = useState<AirlineRegistrationRequest | null>(null);
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true);
      const response = await api.get('/api/v1/airline-approval/requests', {
        params: {
  ...
  const handleApprove = async (requestId: string) => {
    ...
    try {
      setApproving(true);
      const response = await api.post(`/api/v1/airline-approval/requests/${requestId}/approve`);
  ...
  const handleReject = async (requestId: string) => {
    ...
    try {
      setRejecting(true);
      const response = await api.post(`/api/v1/airline-approval/requests/${requestId}/reject`, {
  ...
  // Si l'utilisateur n'est pas support, ne rien afficher (sera redirigé)
  if (!user || user.role !== 'support') {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600">Seuls les utilisateurs support peuvent accéder à cette page.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-2">
            ...
            Gestion des Approbations Airlines
          </h1>
          <p className="text-gray-600 mt-1">Approuver ou rejeter les demandes d'inscription des compagnies aériennes</p>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Vous êtes sur le point de rejeter la demande de <strong>{selectedRequest.name}</strong> ({selectedRequest.code}) - {selectedRequest.email}
            </p>
            <div className="mb-4">
