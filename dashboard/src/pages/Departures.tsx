...
export default function Departures() {
  ...
  useEffect(() => {
    if (lastUpdate > 0) fetchPassengers();
  ...
  const periodFilteredPassengers = useMemo(() => {
    return departureFlights.filter(pax => {
  ...
  }, [departureFlights, periodFilter, customDate]);
  ...
  if (loading && !error) {
    return <LoadingPlane text="Chargement des départs..." size="md" />;
  }

  return (
    <div className="space-y-6 p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-white">Vols en Départ - {user?.airport_code}</h1>
        <button
      ...
      <div className="grid grid-cols-5 gap-4">
        ...
        <div className="bg-green-500/20 border border-green-500/30 rounded-lg p-4">
          <p className="text-green-300/70 text-sm">Embarqués</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/10">
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-white/50 uppercase">Embarquement</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-white/50 uppercase">Bagages</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
