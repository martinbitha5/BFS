import { CheckCircle, Clock, XCircle, AlertCircle, UserCheck, UserX, ShieldAlert } from 'lucide-react';
...
interface RegistrationRequest {
  ...
  role: 'supervisor' | 'baggage_dispute';
...
export default function UserApproval() {
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true);
      const response = await api.get('/api/v1/user-approval/requests', {
        params: {
  ...
  const handleApprove = async (requestId: string) => {
    ...
    try {
      setApproving(true);
      const response = await api.post(`/api/v1/user-approval/requests/${requestId}/approve`);
  ...
  const handleReject = async (requestId: string) => {
    ...
    try {
      setRejecting(true);
      const response = await api.post(`/api/v1/user-approval/requests/${requestId}/reject`, {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Gestion des Approbations</h1>
          <p className="text-gray-600 mt-1">Approuver ou rejeter les demandes d'inscription</p>
        </div>
      </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {requests.map((request) => (
                <tr key={request.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="text-sm text-gray-900">{getRoleLabel(request.role)}</span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="text-sm text-gray-900">
                      {request.airport_code === 'ALL' ? 'Tous les aéroports' : request.airport_code || 'N/A'}
                    </span>
                  </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Vous êtes sur le point de rejeter la demande de <strong>{selectedRequest.full_name}</strong> ({selectedRequest.email})
            </p>
            <div className="mb-4">
